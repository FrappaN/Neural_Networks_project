...
# We redefine the CaptumExplainer class from Pytorch Geometric to include the GradientShap method,
# which is not included in the library
# The choice of n_samples = 1  follows the original implementation of the paper, while we pass the baseline 
# as an argument to perform experiments with different baselines.
...
class CaptumExplainer(ExplainerAlgorithm):
    ...
    * :class:`captum.attr.GuidedBackprop`
    ...
    def _is_supported_attribution_method(self) -> bool:
        ...
        if self.attribution_method_class.__name__ in self.SUPPORTED_METHODS:
            return True
        return False
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self._needs_baseline():
            if self._baseline == 'zeros':
                baseline = torch.zeros_like(inputs[0])
            elif self._baseline == 'ones':
                baseline = torch.ones_like(inputs[0])
            else:
                raise ValueError(f"Unsupported baseline type '{self._baseline}'")
